...
def show_pcl(pcl):
    ...
    vis.create_window('LiDAR PointCloud',width=640, height=640,)
    ...
    #  keep window open until right-arrow is pressed (key-code 262)
...
def bev_from_pcl(lidar_pcl, configs, vis: bool=False):
    ...
    _, idxs_unique, counts = np.unique(lidar_pcl_top[:, 0:2], axis=0, return_index=True, return_counts=True)
    lidar_pcl_top = lidar_pcl_top[idxs_unique]
    ...
    if vis:
        ...
        # img_intensity = cv2.convertScaleAbs(img_intensity, alpha=alpha, beta=beta)  # call convertScaleAbs function for better visualization
    ...
    if vis:
        ...
        while (1):
            cv2.imshow('height_map', intensity_image_norm)
            if cv2.waitKey(10) & 0xFF == 27:
                break
        cv2.destroyAllWindows()
