...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  auto look_ahead_distance = velocity_mag * _lookahead_time + 0.5 * accel_mag * _lookahead_time * _lookahead_time;  // <- done
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.location.x += _stop_line_buffer * std::cos(ang);  // <- done
      goal.location.y += _stop_line_buffer * std::sin(ang);  // <- done

      // LOG(INFO) << "BP- new STOP goal at: " << goal.location.x << ", "
      //          << goal.location.y;

      // TODO-goal speed at stopping point: What should be the goal speed??
      goal.velocity.x = 0;  // <- done
      goal.velocity.y = 0;  // <- done
      goal.velocity.z = 0;  // <- done
      ...
      goal.velocity.x = _speed_limit * std::cos(goal.rotation.yaw);  // <- done
      goal.velocity.y = _speed_limit * std::sin(goal.rotation.yaw);  // <- done
      goal.velocity.z = 0;
    }
    ...
    // the stop line. goal = ...;
    goal = _goal ;  // Keep previous goal. Stay where you are. // <- done
    ...
    if (stopped_secs >= _req_stop_time && tl_state.compare("Red") != 0) {
      ...
      _active_maneuver = FOLLOW_LANE ;  // <- done
